...
export async function showPreferencesForm(categories: Category[]): Promise<void> {
                    ...
                    <div class="rating-container">
                        ${[2,4,6,8,10].map(score => `
                            <label class="rating-label" title="Rating ${score}">
                                <input type="radio" name="category_${category.id}" value="${score}">
                                <span>${score}</span>
                            </label>
                        `).join('')}
                    </div>
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const preferences: Preference[] = categories.map(category => ({
            category_id: category.id,
            score: parseInt(
                (form.querySelector(`input[name="category_${category.id}"]:checked`) as HTMLInputElement)?.value || '0'
            )
        }));

        if (preferences.some(p => p.score === 0)) {
